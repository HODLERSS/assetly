...
m2 factual accuracy: every number (weights, totals, percentages) is consistent with the ground truth or internally consistent; weights within 2 percentage points and dollar amounts within 1% are fine (prices move between writing and grading; dollars are rounded to the nearest hundred by design), and approximate phrasings of a true figure ("almost 160%" for 158.2%, "about a third" for 34%) are correct, not errors. Qualitative business descriptions count as errors only if plainly false about a well-known company. To FAIL you MUST quote a contradicting pair in m2_evidence; else m2 passes.
