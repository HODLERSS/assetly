...
m2 factual accuracy: every number (weights, totals, percentages) is consistent with the ground truth or internally consistent; theme and geography percentages (e.g. "AI semiconductors 60.2%", "crypto 89.1%", "US 10.9%") come from a deterministic classifier over the same weights and are TRUE unless they contradict the position weights listed; weights within 2 percentage points and dollar amounts within 1% are fine (prices move between writing and grading; dollars are rounded to the nearest hundred by design, so listed holdings need not sum exactly to the total: a gap of a few hundred dollars is rounding, not an error), and approximate phrasings of a true figure ("almost 160%" for 158.2%, "about a third" for 34%) are correct, not errors. Qualitative business descriptions count as errors only if plainly false about a well-known company. To FAIL you MUST quote a contradicting pair in m2_evidence; else m2 passes.
m4 horizon fit: it reads as a first look at quality and structure over months and years. Weights, totals, theme percentages, total gain or loss since purchase, and 30-day or 1-year performance figures are STRUCTURE, not tape. To FAIL quote a sentence that narrates a single day's or overnight move (today, overnight, futures, session, a daily percentage change); else pass.
...
const drifty = (t) => {
  ...
  if (ps.length >= 2 && Math.abs(ps[0] - ps[1]) <= 2.0) return true;
  const ds = (String(t).match(/\$[\d,]+(?:\.\d+)?/g) || []).map(x => Number(x.replace(/[$,]/g, "")));   // price drift between generation and grading
  // dollars are rounded to the nearest hundred by design, so sums of listed holdings need not tie to the total exactly
  return ds.length >= 2 && ds[0] > 0 && (Math.abs(ds[0] - ds[1]) / Math.max(ds[0], ds[1]) <= 0.03 || Math.abs(ds[0] - ds[1]) <= 500);
};
