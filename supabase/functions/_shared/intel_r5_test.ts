...
Deno.test("windows: 'today' is the market's day, and the previous day until that market opens (the chart's rule)", () => {
  // Samsung at 20:00 UTC Sep 21 = 05:00 KST Sep 22, before KRX opens: still Sep 21, so 1Y starts Sep 21, 2025
  const sam = Date.parse("2026-09-21T20:00:00Z");
  assertEquals(marketToday("KR", sam), "2026-09-21");
  assertEquals(windowTargetYmd(365, sam, "KR"), "2025-09-21");
  // after KRX opens (00:30 UTC = 09:30 KST) it is Sep 22
  assertEquals(windowTargetYmd(365, Date.parse("2026-09-22T00:30:00Z"), "KR"), "2025-09-22");
  ...
  // Samsung's 1Y base at 20:00 UTC Sep 21 is the Sep 19, 2025 close (Sep 21, 2025 was a Sunday), never the Sep 22 one
  const h = [
    { ts: "2025-09-19T06:30:00Z", price: 100 }, { ts: "2025-09-22T06:30:00Z", price: 103.3 },
    { ts: "2026-09-21T06:30:00Z", price: 331.6 },
  ];
