...
desk_view: STRUCTURE, exactly two or three sentences: first the concentration, correlation, currency, or leverage fact that most shapes this book, with its percentage from the data; then a sentence starting "This means" that says what it does FOR and TO them - if the concentration fits their stated risk appetite and target, that sentence names the upside it is buying before the condition that would break it; if it does not fit, it names what it has delivered before what it puts at risk. No performance figures here (they belong in the notes), no list of returns, no single-day numbers. <= 50 words. Never invent a hypothetical loss or drawdown percentage.
